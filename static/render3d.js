...
const PAL = {
  ...
  // Верх стены: 2D красит его #454d63 и добавляет светлую кромку #666f88
  // только там, где стена выглядывает из-за соседа. Здесь кромки нет —
  // её роль играет настоящая грань, — поэтому берётся среднее из двух.
  wall: [86, 95, 118],          // #565f76; бока темнее вершинно
...
class Batch {
  ...
  dispose() {
    ...
    this.mesh.dispose();
    this.mesh = null;
    // Геометрия и материал тоже: бэкенды переключают туда-сюда, а каждый
    // init() собирает их заново. Без этого шестое переключение оставило бы
    // в видеопамяти пять комплектов буферов и пять скомпилированных
    // программ. Материал у нескольких пачек общий — повторный dispose()
    // у three.js безвреден (это просто событие).
    this.geo.dispose();
    this.mat.dispose();
  }
}
...
export function createRenderer() {
  ...
  return {
    ...
    // пришлось бы списать это число себе — то есть завести вторую истину
    // о раскладке, которая разойдётся при первой же правке модели тела.
    aimY: ENT_Y,
    ...
    _buildAssets() {
      ...
      // Глубина каймы 0.86 в ЛИНЕЙНОМ пространстве — это множитель 0.94 по
      // sRGB, то есть ровно порядок 2D-сетки rgba(0,0,0,0.16). Пробовал
      // 0.72: получается не сетка, а рябь — кайма у квада с одним делением
      // размазана на полклетки, и на глаз это тёмные пятна, а не границы.
      const floorGeo = gridTile(new THREE.PlaneGeometry(1, 1, 2, 2)
                                  .rotateX(-Math.PI / 2), 0.86);
    ...
    _pushBody(e, k, isSelf) {
      ...
      if (this.combat && !dead && k.bar && e.hpMax > 0 && e.hp < e.hpMax) {
        ...
        // ВЫСОТА ПОЛОСКИ ВЫВЕДЕНА, А НЕ ПОДОБРАНА. Тело на экране кончается
        // не на своей высоте, а на высоте плюс половина глубины
        // основания: BODY_H*48*cos(57°) + 0.70*48*sin(57°)/2 = 9.4 + 14.1 =
        // 23.5 px над точкой ног. Полоска сама 0.16 клетки, то есть ±3.8 px
        // от своего центра. Значит центр обязан стоять выше 27.3 px, а это
        // (27.3/48)/cos(57°) = 1.05 клетки. При прежних 0.53 полоска
        // ложилась ПРЯМО НА КРЫШКУ тела — замечено на кадре: враг выглядел
        // чашкой с тёмным нутром вместо диска.
        const by = 1.06;
        q4.copy(Q_BILLBOARD);
