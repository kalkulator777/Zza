/* Canvas-рендер боя: интерполяция снапшотов, частицы, HUD. */
const Render = {
  S: 0.78, CW: 1600, CH: 900, WCX: 800, WCY: 480,
  DELAY: 90,
  cv: null, ctx: null, arena: null, players: {}, heroes: {}, myPid: null,
  buf: [], parts: [], shake: 0, t: 0, beams: [], aimLine: null, running: false,
  ...
  setMatch(msg, myPid) {
    this.arena = msg.arena; this.myPid = myPid;
    this.players = {};
  ...
  view() {
    ...
    const byId = arr => { const m = {}; arr.forEach(x => m[x.i] = x); return m; };
    const pb = byId(b.p), ob = byId(b.o);
    const lerp = (x, y) => x + (y - x) * k;
    const p = a.p.map(x => {
      const y = pb[x.i]; if (!y) return x;
      return Object.assign({}, y, { x: lerp(x.x, y.x), y: lerp(x.y, y.y) });
    });
    b.p.forEach(y => { if (!a.p.some(x => x.i === y.i)) p.push(y); });
    const o = b.o.map(y => {
      const x = (a.o || []).find(q => q.i === y.i);
      return x ? Object.assign({}, y, { x: lerp(x.x, y.x), y: lerp(x.y, y.y) }) : y;
    });
    return { s: b, p, o };
  },

  /* ---------- частицы ---------- */
  add(p) { if (this.parts.length < 700) this.parts.push(p); },
  spark(x, y, col, n, sp, life) {
    for (let i = 0; i < n; i++) {
      const a = Math.random() * 6.283, v = (0.4 + Math.random()) * (sp || 260);
      this.add({ k: 'p', x, y, vx: Math.cos(a) * v, vy: Math.sin(a) * v - 40, c: col, l: life || 0.4, m: life || 0.4, r: 1.5 + Math.random() * 2 });
    }
  },
  ring(x, y, r0, r1, col, life, w) {
    this.add({ k: 'r', x, y, r0, r1, c: col, l: life, m: life, w: w || 3 });
  },
  ...
  heroCol(pid) {
    const p = this.players[pid]; const h = p && this.heroes[p.hero];
  ...
  onEvent(e, snap) {
    ...
    if (k === 'hit') {
      ...
      this.arc(e.x, e.y, e.d || 1, e.s || 1, '#fff');
    } else if (k === 'dashslash') {
      this.ring(e.x, e.y, 10, 90, '#ff8ea0', 0.3, 4);
    } else if (k === 'whirl') {
      this.ring(e.x, e.y, 20, 115 * (e.s || 1), '#ff4d6d', 0.35, 5);
    } else if (k === 'bash' || k === 'sparks') {
      this.spark(e.x, e.y, '#ffd166', 10, 320, 0.35);
      this.arc(e.x, e.y, e.d || 1, e.s || 1, '#ffe6a8');
    } else if (k === 'ram') {
      this.ring(e.x, e.y, 20, 80, '#f4a259', 0.3, 6);
    } else if (k === 'guard') {
      this.ring(e.x, e.y, 30, 60, '#f4a259', 0.3, 3);
    } else if (k === 'cone') {
      this.arc(e.x + 70 * (e.d || 1), e.y, e.d || 1, 1.8, '#bff0ff');
    } else if (k === 'shock') {
      this.ring(e.x, e.y, 10, e.r || 170, '#b58cff', 0.45, 6);
    } else if (k === 'boom') {
      ...
      this.ring(e.x, e.y, 90, 10, this.heroCol(e.p), 0.45, 3);
    } else if (k === 'jump') {
      this.spark(e.x, e.y, '#6b7590', 4, 90, 0.25);
    } else if (k === 'dash') {
      this.ring(e.x, e.y, 8, 46, '#9fd8ff', 0.22, 3);
    } else if (k === 'shieldhit' || k === 'shieldon') {
      this.ring(e.x, e.y, 12, 52, '#8ad7ff', 0.32, 3);
    } else if (k === 'pdie') {
      this.spark(e.x, e.y, '#9aa6c0', 4, 120, 0.25);
    } else if (k === 'crit') {
      this.ring(e.x, e.y, 6, 40, '#fff', 0.25, 3);
    } else if (k === 'bang') {
      this.ring(e.x, e.y, 6, 30 + 60 * (e.m || 0), '#5bc8ff', 0.25, 4);
    } else if (k === 'charge') {
      this.ring(e.x, e.y, 60 - 40 * (e.m || 0), 20, '#5bc8ff', 0.2, 2);
    } else if (k === 'beamaim') {
      this.aimLine = { x: e.x, y: e.y, dx: e.dx, dy: e.dy, t: performance.now() };
    } else if (k === 'beam') {
      this.beam(e.x, e.y, e.x + e.dx * 2600, e.y + e.dy * 2600, '#5bc8ff', 0.35, 16);
    } else if (k === 'ultflash') {
      this.ring(e.x, e.y, 10, 220, e.c || '#fff', 0.5, 7);
    } else if (k === 'snow') {
      for (let i = 0; i < 5; i++) {
        const a = Math.random() * 6.283, r = Math.random() * (e.r || 100);
        this.add({ k: 'p', x: e.x + Math.cos(a) * r, y: e.y + Math.sin(a) * r - 60, vx: 0, vy: 90, c: '#dff6ff', l: 1.1, m: 1.1, r: 2 });
      }
    } else if (k === 'pulse') {
      this.ring(e.x, e.y, 20, e.r || 150, '#6ee7a0', 0.5, 2);
    } else if (k === 'pull') {
      this.ring(e.x, e.y, 40, 8, '#ffd166', 0.3, 3);
    } else if (k === 'build') {
      this.ring(e.x, e.y, 40, 12, '#ffd166', 0.3, 2);
    } else if (k === 'revive') {
      this.ring(e.x, e.y, 8, 90, '#b58cff', 0.5, 5);
    }
  },
  ...
  step(dt) {
    ...
    for (let i = this.beams.length - 1; i >= 0; i--) {
      this.beams[i].l -= dt; if (this.beams[i].l <= 0) this.beams.splice(i, 1);
    }
  ...
  frame(dt) {
    ...
    const sh = this.shake;
    const sx = (Math.random() - .5) * sh * 2, sy = (Math.random() - .5) * sh * 2;
    c.save();
    ...
    if (v) {
      ...
      this.drawProjectiles(c, v.o || [], v.p);
      v.p.forEach(p => { if (p.al) this.drawFighter(c, p); });
      this.drawParts(c);
    }
    c.restore();
  ...
  bg(c) {
    ...
    c.save(); c.globalAlpha = .25; c.strokeStyle = (a && a.accent) || '#3d5a80'; c.lineWidth = 1;
    const off = (this.t * 8) % 80;
    c.beginPath();
    for (let x = -80 + off; x < this.CW + 80; x += 80) { c.moveTo(x, 0); c.lineTo(x, this.CH); }
    for (let y = -80 + off; y < this.CH + 80; y += 80) { c.moveTo(0, y); c.lineTo(this.CW, y); }
    c.stroke(); c.restore();
    ...
    c.fillStyle = vg; c.fillRect(0, 0, this.CW, this.CH);
  },

  drawBlast(c) {
    const b = this.arena && this.arena.blast; if (!b) return;
    c.save(); c.strokeStyle = 'rgba(255,80,90,.35)'; c.lineWidth = 3; c.setLineDash([16, 14]);
  ...
  drawPlatforms(c) {
    ...
    a.platforms.forEach(p => {
      if (p.oneway) {
        ...
        g.addColorStop(0, hexA(a.accent, .95)); g.addColorStop(.12, hexA(a.accent, .5));
        g.addColorStop(1, 'rgba(6,8,14,.95)');
        c.fillStyle = g; this.rr(c, p.x, p.y, p.w, p.h, 8); c.fill();
        c.fillStyle = 'rgba(255,255,255,.22)'; c.fillRect(p.x + 6, p.y, p.w - 12, 4);
      }
    });
  },
  ...
  drawZones(c, zs) {
    zs.forEach(z => {
      const col = { dome: '#f4a259', blizzard: '#bff0ff', field: '#6ee7a0', mark: '#ff5b6e' }[z.k] || '#fff';
      c.save();
      if (z.k === 'mark') {
        const k = 1 - z.p;
        c.strokeStyle = col; c.lineWidth = 4; c.globalAlpha = .5 + .5 * Math.sin(this.t * 22);
        c.beginPath(); c.arc(z.x, z.y, z.r, 0, 6.283); c.stroke();
        c.globalAlpha = .3; c.beginPath(); c.arc(z.x, z.y, z.r * (1 - k), 0, 6.283);
        c.fillStyle = col; c.fill();
      } else {
        c.globalAlpha = .13; c.fillStyle = col;
        c.beginPath(); c.arc(z.x, z.y, z.r, 0, 6.283); c.fill();
        c.globalAlpha = .5; c.strokeStyle = col; c.lineWidth = 2;
  ...
  drawStructs(c, bs) {
    bs.forEach(s => {
      ...
      if (s.k === 'turret') {
        ...
        c.fillStyle = '#2a3145'; this.rr(c, -s.w / 2, 0, s.w, s.h / 2, 4); c.fill();
        c.strokeStyle = tc; c.lineWidth = 3;
        c.save(); c.rotate(-(s.d || 0) * Math.PI / 180);
        c.beginPath(); c.moveTo(0, -4); c.lineTo(26, -4); c.stroke(); c.restore();
        c.fillStyle = '#ffd166'; c.beginPath(); c.arc(0, -4, 9, 0, 6.283); c.fill();
        ...
        c.translate(s.x + s.w / 2, s.y + s.h / 2);
        c.fillStyle = '#39405a'; c.beginPath(); c.arc(0, 0, 10, 0, 6.283); c.fill();
        ...
        c.beginPath();
        for (let i = 1; i < 4; i++) { c.moveTo(s.x, s.y + s.h * i / 4); c.lineTo(s.x + s.w, s.y + s.h * i / 4 - 6); }
        c.stroke();
      }
      c.restore();
    });
  ...
  drawProjectiles(c, os, ps) {
    os.forEach(o => {
      ...
      if (o.k === 'hook') {
        ...
        c.fillStyle = '#ffd166'; c.beginPath(); c.arc(0, 0, 8, 0, 6.283); c.fill();
      } else if (o.k === 'beam') {
        c.rotate(-a); c.fillStyle = 'rgba(120,220,255,.9)';
        c.fillRect(-90, -o.r, 180, o.r * 2);
      } else {
        const col = { bolt: '#9fe4ff', slug: '#5bc8ff', ice: '#bff0ff', orb: '#c9a8ff', bolt2: '#ffd166' }[o.k] || '#fff';
        c.rotate(-a);
        c.shadowColor = col; c.shadowBlur = 14;
        c.fillStyle = col;
        if (o.k === 'ice') {
          c.beginPath(); c.moveTo(o.r * 1.6, 0); c.lineTo(0, -o.r); c.lineTo(-o.r * 1.6, 0); c.lineTo(0, o.r);
          c.closePath(); c.fill();
        } else {
          this.rr(c, -o.r * 1.8, -o.r, o.r * 3.6, o.r * 2, o.r); c.fill();
        }
        c.globalAlpha = .25; c.fillRect(-o.r * 8, -o.r * .4, o.r * 6, o.r * .8);
      }
      c.restore();
    });
  },
  ...
  drawFighter(c, p) {
    ...
    if (p.iv) c.globalAlpha = .45 + .35 * Math.sin(this.t * 26);

    // тело
    const g = c.createLinearGradient(0, y - H / 2, 0, y + H / 2);
    g.addColorStop(0, col); g.addColorStop(1, hexA(col, .45));
    c.fillStyle = g;
    ...
    c.fillStyle = tc;
    c.fillRect(x + (p.f > 0 ? 12 : -18) + 2, y - H / 2 + 13, 5, 6);

    // глиф
    const gl = GLYPH[info.hero];
    if (gl) { c.globalAlpha *= .9; gl(c, x, y + 10, 30, 'rgba(12,14,22,.9)'); c.globalAlpha /= .9; }

    // состояния
    if (p.a === 'dash') { c.strokeStyle = '#9fd8ff'; c.lineWidth = 2; c.globalAlpha = .5; c.stroke(); c.globalAlpha = 1; }
    c.restore();
    ...
    if (ef.includes('freeze')) {
      c.fillStyle = 'rgba(160,225,255,.35)'; this.rr(c, x - W / 2 - 4, y - H / 2 - 4, W + 8, H + 8, 8); c.fill();
    ...
    if (ef.includes('stun')) {
      for (let i = 0; i < 3; i++) {
        ...
        c.fillStyle = '#ffd166';
        c.beginPath(); c.arc(x + Math.cos(a) * 20, y - H / 2 - 12 + Math.sin(a) * 5, 3, 0, 6.283); c.fill();
      }
    }
    if (ef.includes('dr')) { c.strokeStyle = 'rgba(244,162,89,.8)'; c.lineWidth = 2; c.beginPath(); c.arc(x, y, 40, 0, 6.283); c.stroke(); }
    if (ef.includes('slow')) { c.fillStyle = 'rgba(140,215,255,.18)'; this.rr(c, x - W / 2, y - H / 2, W, H, 11); c.fill(); }
    c.restore();
    ...
    c.fillStyle = 'rgba(0,0,0,.6)'; this.rr(c, x - bw / 2, y - H / 2 - 13, bw, 6, 3); c.fill();
    c.fillStyle = hpk > .35 ? tc : '#ff4d6d';
    this.rr(c, x - bw / 2, y - H / 2 - 13, bw * hpk, 6, 3); c.fill();
    if (p.sh > 0) {
      c.fillStyle = 'rgba(200,240,255,.9)';
      this.rr(c, x - bw / 2, y - H / 2 - 16, bw * Math.min(1, p.sh / 40), 3, 2); c.fill();
    }
    if (me) {
      c.fillStyle = '#fff'; c.globalAlpha = .8;
      c.beginPath(); c.moveTo(x, y - H / 2 - 26); c.lineTo(x - 6, y - H / 2 - 35); c.lineTo(x + 6, y - H / 2 - 35);
      c.closePath(); c.fill();
  ...
  drawParts(c) {
    this.beams.forEach(b => {
      const k = b.l / b.m;
      c.save(); c.globalAlpha = k; c.strokeStyle = b.c; c.lineWidth = b.w * k;
      c.shadowColor = b.c; c.shadowBlur = 30;
      c.beginPath(); c.moveTo(b.x1, b.y1); c.lineTo(b.x2, b.y2); c.stroke(); c.restore();
    });
    if (this.aimLine && performance.now() - this.aimLine.t < 120) {
      const a = this.aimLine;
      c.save(); c.globalAlpha = .45; c.strokeStyle = '#ff6b6b'; c.lineWidth = 2; c.setLineDash([10, 10]);
      c.beginPath(); c.moveTo(a.x, a.y); c.lineTo(a.x + a.dx * 2600, a.y + a.dy * 2600); c.stroke(); c.restore();
    }
    this.parts.forEach(p => {
      const k = p.l / p.m;
      c.save();
      if (p.k === 'p') {
        c.globalAlpha = k; c.fillStyle = p.c;
        c.beginPath(); c.arc(p.x, p.y, p.r * k + .5, 0, 6.283); c.fill();
      } else if (p.k === 'r') {
        const r = p.r0 + (p.r1 - p.r0) * (1 - k);
        c.globalAlpha = k * .9; c.strokeStyle = p.c; c.lineWidth = p.w * k + .5;
        c.beginPath(); c.arc(p.x, p.y, Math.max(1, r), 0, 6.283); c.stroke();
      } else if (p.k === 'a') {
        c.globalAlpha = k; c.strokeStyle = p.c; c.lineWidth = 7 * k + 1;
        c.translate(p.x, p.y); c.scale(p.d, 1);
        c.beginPath(); c.arc(0, 0, 48 * p.s, -1.0, 1.0); c.stroke();
      } else if (p.k === 'n') {
        c.globalAlpha = Math.min(1, k * 2); c.fillStyle = p.c; c.textAlign = 'center';
        c.font = `800 ${p.s}px "Segoe UI",sans-serif`;
        c.strokeStyle = 'rgba(0,0,0,.8)'; c.lineWidth = 4;
        c.strokeText(p.t, p.x, p.y); c.fillText(p.t, p.x, p.y);
      }
      c.restore();
    });
  },
  ...
  drawHUD(c, v) {
    const s = v.s;
    const byTeam = [[], []];
    v.p.forEach(p => { const i = this.players[p.i]; if (i) byTeam[i.team].push(p); });

    byTeam[0].forEach((p, i) => this.card(c, 22, 22 + i * 66, p, 1));
    byTeam[1].forEach((p, i) => this.card(c, this.CW - 22 - 320, 22 + i * 66, p, -1));

    // таймер
    c.save(); c.textAlign = 'center'; c.font = '700 22px "Segoe UI",sans-serif';
    c.fillStyle = 'rgba(230,235,245,.8)';
    const tl = Math.max(0, s.tl || 0);
    c.fillText(`${String(Math.floor(tl / 60)).padStart(2, '0')}:${String(Math.floor(tl % 60)).padStart(2, '0')}`, this.CW / 2, 40);
    c.restore();
  ...
  briefing(c, p) {
    const info = this.players[p.i] || {}; const hero = this.heroes[info.hero] || {};
    const abs = hero.abilities || [];
    const parts = abs.map(a => [KEYLABEL[a.key], a.name])
      .concat([['SHIFT', 'Рывок']]);
    c.save();
  ...
  card(c, x, y, p, dir) {
    ...
    const gx = dir > 0 ? x + 30 : x + 290;
    const gl = GLYPH[info.hero]; if (gl) gl(c, gx, y + 28, 30, hero.color || '#fff');
    const bx = dir > 0 ? x + 56 : x + 22;
    c.textAlign = 'left';
    c.font = '600 14px "Segoe UI",sans-serif'; c.fillStyle = '#dfe6f5';
    ...
    if (p.sh > 0) { c.fillStyle = 'rgba(200,240,255,.95)'; this.rr(c, bx, y + 24, w * Math.min(1, p.sh / 40), 11, 5); c.fill(); }
    c.font = '700 11px "Segoe UI",sans-serif'; c.fillStyle = 'rgba(255,255,255,.85)';
    c.fillText(Math.ceil(p.hp) + (p.sh > 0 ? ' +' + Math.ceil(p.sh) : ''), bx + 4, y + 33);
    // ульта
    c.fillStyle = 'rgba(0,0,0,.55)'; this.rr(c, bx, y + 38, w, 5, 2); c.fill();
    c.fillStyle = p.u >= 100 ? '#ffd166' : '#8ab4ff';
    this.rr(c, bx, y + 38, w * Math.min(1, p.u / 100), 5, 2); c.fill();
    // жизни
    ...
    if (!p.al) {
      c.fillStyle = 'rgba(0,0,0,.55)'; this.rr(c, x, y, 320, 56, 10); c.fill();
      c.fillStyle = '#ff9aab'; c.font = '700 16px "Segoe UI",sans-serif'; c.textAlign = 'center';
  ...
  selfBar(c, p) {
    ...
    const abs = hero.abilities || [];
    const n = keys.length + 1, bw = 74, gap = 10;
    const total = n * bw + (n - 1) * gap;
    let x = this.CW / 2 - total / 2, y = this.CH - 104;
    c.save();
    keys.forEach((k, i) => {
      const ab = abs.find(a => a.key === k) || {};
      const cd = cds[k], maxcd = ab.cd || 1;
      const ready = k === 'r' ? p.u >= 100 : cd <= 0;
      c.fillStyle = 'rgba(10,13,22,.72)'; this.rr(c, x, y, bw, bw, 12); c.fill();
      c.strokeStyle = ready ? (hero.color || '#fff') : 'rgba(120,130,155,.5)';
      c.lineWidth = ready ? 2.5 : 1.5; c.stroke();
      c.save(); c.globalAlpha = ready ? 1 : .35;
      ...
      if (!ready) {
        if (k === 'r') {
          ...
          c.fillStyle = '#ffd166'; c.textAlign = 'center'; c.font = '700 15px "Segoe UI",sans-serif';
          c.fillText(Math.floor(p.u) + '%', x + bw / 2, y + bw - 12);
        } else {
          c.fillStyle = 'rgba(0,0,0,.6)';
          this.rr(c, x, y, bw, bw * Math.min(1, cd / maxcd), 12); c.fill();
          c.fillStyle = '#fff'; c.textAlign = 'center'; c.font = '800 22px "Segoe UI",sans-serif';
          c.fillText(cd.toFixed(1), x + bw / 2, y + bw / 2 + 8);
        }
      }
      c.fillStyle = 'rgba(230,236,250,.9)'; c.textAlign = 'center';
    ...
    const dcd = p.cd[2];
    c.fillStyle = 'rgba(10,13,22,.72)'; this.rr(c, x, y, bw, bw, 12); c.fill();
    c.strokeStyle = dcd <= 0 ? '#9fd8ff' : 'rgba(120,130,155,.5)'; c.lineWidth = dcd <= 0 ? 2.5 : 1.5; c.stroke();
    c.fillStyle = dcd <= 0 ? '#9fd8ff' : 'rgba(150,160,185,.5)'; c.textAlign = 'center';
    c.font = '800 26px "Segoe UI",sans-serif'; c.fillText('»', x + bw / 2, y + bw / 2 + 8);
    c.font = '700 11px "Segoe UI",sans-serif'; c.fillStyle = 'rgba(230,236,250,.9)';
    c.fillText('SHIFT', x + bw / 2, y + bw - 6);
    if (dcd > 0) {
      c.fillStyle = 'rgba(0,0,0,.6)'; this.rr(c, x, y, bw, bw * Math.min(1, dcd / 2.2), 12); c.fill();
    }
    c.restore();
  }
};
