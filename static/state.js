...
// Стрелок (DESIGN.md 4.2, бестиарий). Отдельное ЗНАЧЕНИЕ kind, а не бит в
// flags: «кто это» и «что он сейчас делает» смешивать нельзя. Пятое
// значение — первое свободное после K_SHOT.
//
// КЛИЕНТ ОБЯЗАН РАБОТАТЬ И БЕЗ НЕГО. Пока сервер шлёт обоих врагов одним
// kind = K_ENEMY, эта строка просто не встречается на проводе, и всё
// рисуется как раньше; незнакомый kind (любое другое число) рендер тоже
// переживает — в таблице видов есть KIND_DEF.
export const K_ENEMY_RANGED = 5;
export const K_BOSS = 10;
...
// сущности по корреляции ввода, на отрисовку не влияет. Порог опознания —
// 0.4 этого числа, так что важна не точность, а порядок: сущность, которая
// «бежит», от сущности, которая стоит.
const SPEED_RUN = 7.5;
