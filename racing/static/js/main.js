...
// ---------------------------------------------------------------------------
//
// Зеркало таблицы server/config.WEATHER_GRIP. Копия здесь по той же причине,
// по которой physics.js целиком повторяет physics.py: клиент считает те же
// шаги, что и сервер, и обязан считать их ТЕМИ ЖЕ числами — иначе своя машина
// каждый снапшот будет оттягиваться реконсиляцией. Меняя число на сервере,
// поменяйте и здесь.
//
// Сам крюк в physics.js числа не знает и знать не должен: он принимает
// state.gripMul снаружи (по умолчанию 1,0), а кто включает погоду, тот
// и кладёт туда значение. Включает её main.js — значит, таблица здесь.
const WEATHER_GRIP = {
    clear: 1.0,
    wet: 0.86,
    snow: 0.72,
