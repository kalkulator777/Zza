...
// Предки, показанные на время прогревающего прохода: переиспользуется,
// в кадре ничего не создаётся.
const _warmShown = [null, null, null, null, null, null, null, null];
// Сколько миллисекунд кадра отсчёта отдаётся прогреву сверх обычной работы.
// Кадр отсчёта стоит 1–3 мс, бюджет кадра 16,7 мс: пять миллисекунд сверху
// оставляют отсчёту две трети кадра и при этом дают дешёвым мешам уходить
// пачкой, а не по одному за кадр. Дорогой меш всё равно занимает кадр
// целиком — цикл выходит по первому же превышению, а не режет компиляцию.
const WARM_BUDGET_MS = 5.0;
// Потолок на весь прогрев, мс. Страховка на незнакомое железо: отсчёт длится
// три секунды, а чистыми обязаны быть только последние 350 мс (см. _armWarmup),
// поэтому дольше двух секунд прогреву работать нельзя ни при каких условиях.
// Если не уложился — остаток очереди бросаем: одиночный рывок в гонке хуже,
// чем дёрганая решётка на старте, но лучше, чем и то и другое.
const WARM_TOTAL_MS = 2000;
...
export class RaceRenderer {
    ...
    constructor(canvas, opts) {
        ...
        // Прогрев шейдеров перед стартом: очередь мешей, которых в спокойном
        // кадре нет, и флаг «объекты программ уже созданы». Пусто без сцены.
        this.warmQueue = null;
        this.warmAt = 0;
        this.warmCompiled = false;
        this.warmSpent = 0;
        this.theme = 'city';
    ...
    update(dt) {
        ...
        if (marks) {
            ...
            marks.fade(step);
            marks.upload(this.renderer, step);
        }
        ...
        // Прогрев идёт последним: камера и отсечение уже посчитаны, и
        // прогревающий проход видит ровно ту же сцену, что настоящий.
        if (this.warmQueue !== null) this._warmStep();
    }

    /**
     * Прогрев шейдеров: собрать меши, до которых спокойный кадр не доходит.
     *
     * Дорога не сборка программы, а ПЕРВАЯ ОТРИСОВКА с ней. Замерено:
     * renderer.compile() создаёт четыре недостающие программы за 5,3 мс, но
     * рывок остаётся — кадр, в котором программа впервые попадает в draw call,
     * стоит 18–22 мс в Firefox, 26 мс в Chromium на llvmpipe и 66–113 мс
     * в Chromium на swiftshader. В гонке это видно как одиночный рывок на
     * 3–6 секунде: там впервые появляется дым заноса (fxSmoke) или ореол фар
     * и стопов (fxGlow). Меши эффектов с нулём экземпляров спрятаны
     * (effects.js), поэтому до первой частицы их программа не рисуется ни разу,
     * и одного compile() мало — это проверено прогоном, а не рассуждением.
     *
     * Лечится тем, что каждый такой меш один раз рисуется заранее, насильно
     * показанным. Прогревающий проход идёт ДО настоящего в том же кадре
     * и затирается им (autoClear), поэтому нулевые матрицы на экран не попадают.
     *
     * ПОЧЕМУ С ПЕРВЫХ КАДРОВ ОТСЧЁТА, А НЕ РАЗМАЗАНО ПО ВСЕМУ ОТСЧЁТУ.
     * Все три секунды отсчёта комната шлёт снапшоты решётки на 20 Гц (12.12),
     * и длинный кадр задерживает их разбор пачкой. Оценку смещения часов это
     * не травит: у таких снапшотов тик заморожен, и метка им ставится по
     * приходу (12.12), в окно они не идут. Но пачка сминает шкалу истории
     * интерполяции: у полудюжины снапшотов метки встают вплотную.
     *
     * Сколько истории обязано быть целым. Показ чужих отстаёт на
     * INTERP_DELAY = 100 мс, а дальше EXTRAPOLATE_MAX = 250 мс идёт
     * экстраполяция — значит в поиске пары участвуют только последние
     * 350 мс истории, остальное кольцо лежит мёртвым грузом. Отсюда
     * требование: прогрев обязан закончиться не позже чем за 350 мс до
     * зелёного света. Поэтому он идёт с первого же кадра после сборки
     * сцены и с бюджетом на кадр — размазать его по всему отсчёту было бы
     * ровно наоборот, помятая история пришла бы к самому старту.
     *
     * Замерено на самом медленном, что у нас есть (Chromium + swiftshader):
     * прогрев занимает 4 кадра и 1,07 с, последний дорогой кадр приходится
     * на 1,6 с отсчёта, то есть чистыми остаются 0,9–1,0 с — втрое больше
     * необходимого. Шаг метки истории у последних десяти снапшотов перед
     * стартом 49–51 мс, в гонке медиана 50,0 мс и ни одной сплющенной.
     */
    _armWarmup() {
        const q = [];
        ...
        this.scene.traverse((o) => {
            ...
            if (o.visible && !(o.isInstancedMesh && o.count === 0)) return;
            // Программа привязана к материалу, а не к мешу: двум мешам
            // с общим материалом хватит одного прохода.
            const m = o.material;
            const key = m.uuid !== undefined ? m.uuid : (m[0] ? m[0].uuid : null);
            if (key === null || key === undefined || seen[key]) return;
            seen[key] = 1;
            q.push(o);
        });
        this.warmQueue = q.length > 0 ? q : null;
        this.warmAt = 0;
        this.warmCompiled = false;
        this.warmSpent = 0;
    }
    ...
    /**
     * Шаг прогрева за кадр: дешёвые меши уходят пачкой, дорогой — один.
     *
     * Почему с бюджетом, а не строго по одному за кадр: очередь состоит
     * в основном из мешей, чей материал уже прогрет (пустые пулы частиц,
     * спрятанные болванки), и каждый такой стоит только лишнего прохода.
     * По одному за кадр прогрев растягивался на 2,0 с отсчёта из 2,6 —
     * то есть ровно до старта, чего допускать нельзя (см. _armWarmup).
     * С бюджетом он укладывается в первую секунду.
     */
    _warmStep() {
        const t0 = now();
        do {
            this._warmOne();
        } while (this.warmQueue !== null && now() - t0 < WARM_BUDGET_MS);
        this.warmSpent += now() - t0;
        if (this.warmSpent > WARM_TOTAL_MS) this.warmQueue = null;
    }

    /** Одна запись очереди прогрева. */
    _warmOne() {
        if (!this.warmCompiled) {
            // Объекты программ разом: 5 мс, и заодно покрывает видимые меши,
            // которые в этом кадре не попали в пирамиду видимости.
            this.renderer.compile(this.scene, this.camera);
            this.warmCompiled = true;
            return;
        }
        const q = this.warmQueue;
        const o = q[this.warmAt++];
        if (this.warmAt >= q.length) this.warmQueue = null;
        if (o === undefined || o.parent === null) return;

        // Показать сам меш и всех невидимых предков: иначе проход до него
        // не дойдёт и прогрев молча не случится.
        const wasCount = o.isInstancedMesh ? o.count : -1;
        if (wasCount === 0) o.count = 1;
        const shown = _warmShown;
        let n = 0;
        for (let p = o; p !== null && n < shown.length; p = p.parent) {
            if (!p.visible) { p.visible = true; shown[n++] = p; }
        }
        this.renderer.render(this.scene, this.camera);
        while (n > 0) { shown[--n].visible = false; shown[n] = null; }
        if (wasCount === 0) o.count = 0;
    }
