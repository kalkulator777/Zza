...
// `night.k` поднят с 0.72 до 0.90 по жалобе «сделать ночь чуть светлее»
// (плейтест 12.14): полотно запекается один раз, поэтому дороже это не
// стоит ни кадра. Само по себе это даёт скромную прибавку — основная часть
// ночной темноты набегает от запечённого света (см. bakeVertexLight и
// CITY_NIGHT/MOUNTAIN_NIGHT/INDUSTRIAL_NIGHT в scenery.js, где яркость
// ambient/directional тоже поднята, тем же поводом), а этот множитель
// довешивает собственно цвет асфальта, бордюров и рельефа. Тон и холодность
// (tint) не трогались: ночь обязана остаться ночью, а не сумерками — она
// по-прежнему заметно темнее и холоднее day/dusk по цвету неба, тумана и
// освещённости объектов, светлеет только сама база.
const TOD_SURFACE = {
    day: { k: 1.0, tint: [1.0, 1.0, 1.0], lineK: 1.0, lineEmissive: null },
    dusk: { k: 0.9, tint: [1.04, 0.96, 0.92], lineK: 1.05, lineEmissive: '#1a1712' },
    night: { k: 0.90, tint: [0.84, 0.9, 1.06], lineK: 1.25, lineEmissive: '#31353d' }
};
