...
def resolve_collisions(cars, stats, count, skip_before=0):
    ...
    первых элементов участвует. ``skip_before`` пропускает пары, в которых
    ОБА участника лежат раньше этого индекса: при physics=rapier машина
    против машины уже посчитана солвером внутри шага, и повторять её здесь
    значило бы толкнуть дважды, а болванки потока в мире модуля не живут и
    свой проход всё равно нужен (§12.28). Ноль — прежнее поведение. Расталкивание и обмен импульсом делятся
