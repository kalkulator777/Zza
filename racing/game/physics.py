...
2. **Темп накопления растёт со скоростью.** Было ``drift_charge += dt``,
   ...
   прямая: виляние держится на 13..16 м/с и получает 0,2..0,4 темпа, честная
   шпилька идёт на 21 м/с и получает 0,87, длинная дуга на 24+ м/с — полный.
   Разница в 4..5 раз делает медленное виляние бессмысленным даже в одиночку,
   без пункта 1. Квадрат считается умножением: ``pow`` в шаге по-прежнему нет.
...
HANDBRAKE_CHARGE_FULL_SPEED = 22.0   # м/с, выше заряд копится в полный темп
