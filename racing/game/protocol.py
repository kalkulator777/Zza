...
#     u8 height_q (высота над полотном по 2 см, 0..5.10 м; 0 — колёса на земле)
#   u8 proj_count
...
# Делений байта высоты на метр. Пара к HEIGHT_SCALE в static/js/protocol.js:
# числа обязаны быть взаимно обратными, иначе сервер и клиент разойдутся в
# высоте прыжка. Сверяется проверкой в tools/test_sim.py.
HEIGHT_PER_METER = 50.0


def quantize_height(height):
    """Высота над полотном в метрах -> байт по 2 см (потолок 5.10 м).

    Отдельный бит «в полёте» во флагах не заводится: все восемь заняты
    (раздел 5.3), а для рендера достаточно самой высоты — ноль означает
    колёса на земле.

    ЦЕНА ДЕЛЕНИЯ БЫЛА 1 СМ, И ПОТОЛКА 2,55 М НЕ ХВАТАЛО НИКОМУ (§12.30).
    Это не Rapier сломал — это было сломано с этапа 2, и никто не заметил,
    потому что смотрели на СВОЮ машину, а её высоту снапшот не возит вовсе:
    её считает предсказание. Замер на 40 м/с: у классики пик прыжка
    2,71–2,84 м, то есть байт упирался в потолок на 0,32–0,45 с каждого
    прыжка и не показывал соседям до 0,29 м. У Rapier рельеф настоящий и
    тяготение 9,81 против аркадных 12,0, пик доходит до 3,72 м — потолок
    держался бы до 1,00 с, пряча до 1,15 м.

    Почему 2 см, а не второй байт. Поле чисто визуальное: рендер приподнимает
    модель на height и ниже AIR_MIN_HEIGHT = 0,06 м (renderer.js) не трогает
    её вовсе — то есть три деления шкалы картинка не различает ПО
    ПОСТРОЕНИЮ. Ошибка округления не больше 1 см; один пиксель кадра при
    FOV 62 градуса и 720 строках — это 1 см на 6,7 м, значит чужая машина
    должна подлететь ближе семи метров, чтобы округление стоило хотя бы
    пикселя. Потолок 5,10 м против замеренных пиков даёт запас 1,37x самому
    высокому прыжку каталога (industrial, 3,72 м) и 1,80x классике.
    """
    if height <= 0.0:
        return 0
    q = int(height * HEIGHT_PER_METER + 0.5)
