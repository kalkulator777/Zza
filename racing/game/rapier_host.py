...
class ShadowWorld(object):
    ...
    def __init__(self, sim, wasm_path: str = WASM_PATH):
        ...
        for car in sim.cars:
            ...
            # Гандикап тень НЕ догоняет: мир строится в конструкторе
            # симуляции, а комната накладывает гандикап после него и
            # пересобирает только настоящий мир Rapier (§12.30). Тень гонку
            # не считает, поэтому пересборки ей не завели; если понадобится
            # мерить тенью гандикапнутую машину — сюда приедет car.handicap.
...
# Машина на крыше. У классики такого состояния нет вовсе — курс там
# кинематический, кузов не кувыркается, — а у Rapier кузов настоящий, и
# после жёсткого удара о стену он ложится крышей вниз. Своим ходом оттуда
# не выбраться ни игроку, ни боту: колёса не достают до полотна, газ не
# делает ничего. До 12.28 это никого не задевало, потому что в замерах
# кувырка не случалось; с потоком машин на дороге он случается, и гонка
# после него висит до истечения восьми минут.
#
# Сеть безопасности та же по смыслу, что FELL_THROUGH, и с теми же
# последствиями: машина возвращается на ось и теряет весь ход.
#
# Пороги выведены, а не подобраны, и мерят они СПОСОБНОСТЬ ЕХАТЬ, а не
# наклон кузова. Наклон пробовали первым: порог по вертикали кузова ловил
# машину на крыше (up = -1,0) и пропускал ту, что легла НА БОК и упёрлась в
# стену (замерено: up = +0,17, одно колесо, стоит вечно). Правильная мера —
# сколько колёс достаёт до полотна: контроллер прикладывает и тягу, и
# поворот только через колесо в контакте, значит меньше двух колёс — это
...
class RapierRace(object):
    ...
    * дорога (§12.28) — той же дверью: масло и обломки в ``grip_drop`` и
      ``speed_drop``, завал и удар о болванку в ``shift_*``/``push_*``.
      Тел в мире модуля у них нет: болванку считает старая физика у
      хозяина, и её контакт с гонщиком переводит в дозу
      ``resolve_collisions``.
    """
