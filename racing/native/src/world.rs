...
impl CarTuning {
    ...
    pub const SIM: CarTuning = CarTuning {
        ...
        // Модель шин (§12.32). Пик на 8,3 град, за пиком спад к 72 % пика.
        tire_peak_slip: 0.145,
        tire_tail: 0.72,
        tire_rear_grip: 1.0,
        tire_load_sens: 0.16,
        tire_roll_couple: 0.85,
    };
    ...
    pub const ARCADE: CarTuning = CarTuning {
        ...
        // Модели шин у аркады НЕТ, и ноль здесь — не забывчивость, а
        // выключатель: при `tire_peak_slip == 0` ветка §12.32 не
        // исполняется вовсе, шину считает встроенный контроллер тем же
        // кодом, что и до неё. На этом стоят прежние хэши четырёх движков.
        tire_peak_slip: 0.0,
        tire_tail: 0.0,
        tire_rear_grip: 0.0,
        tire_load_sens: 0.0,
...
/// Пол продольной скорости при счёте угла увода, м/с.
///
/// Выведен из шума, а не подобран. Остаточная боковая скорость машины,
/// едущей ПО ПРЯМОЙ без руля, измерена на всех пяти машинах и не
/// превышает 0,0248 м/с (худшая — Маслкар, 900 тиков разгона до 20 м/с).
/// Порог обязан быть таким, чтобы этот шум не читался шиной как срыв,
/// то есть давал меньше градуса увода: atan(0,0248 / 2,0) = 0,71 град,
/// запас до градуса — 1,4x. Отсюда 2,0 м/с.
///
/// Он же держит машину на месте: на стоянке 10 с снос ровно 0,00000 м —
/// при нулевой боковой скорости кривая даёт ноль силы и шине нечем
/// толкнуть машину в сторону.
pub const TIRE_V_REF: f32 = 2.0;

/// Кривая увода: какую долю пикового сцепления даёт шина на этом угле.
///
/// До пика — четверть синусоиды: ноль в нуле, единица на `peak`, и
/// производная в пике РОВНО ноль, то есть пик настоящий, а не излом.
/// За пиком — спад к полке `tail` (доля пика, которую шина держит в
/// скольжении). Обе ветки сшиты в `s = 1` и по значению, и по производной.
///
/// Именно это отличает симулятор от потолка трения: у встроенного
/// контроллера сила упирается в `friction_slip * Fz` и держится там до
/// любого угла, поэтому предел нечем нащупать. Здесь за пиком сила ПАДАЕТ.
pub fn tire_curve(slip_abs: f32, peak: f32, tail: f32) -> f32 {
    let s = slip_abs / peak;
    if s <= 1.0 {
        (core::f32::consts::FRAC_PI_2 * s).sin()
    } else {
        let d = s - 1.0;
        tail + (1.0 - tail) / (1.0 + d * d)
    }
}
...
pub struct Car {
    ...
    pub drift_level: f32,
    /// тяга на колесо, Н — что шаг 1 заказал модели шин; живёт один шаг
    pub tire_engine: f32,
    /// тормоз на колесо, Н (с ручником на задней оси); живёт один шаг
    pub tire_brake: [f32; 4],
}
...
impl World {
    ...
    pub fn step(&mut self, inputs: &[CarInput], outputs: &mut [CarOut],
        ...
        for i in 0..n {
            ...
            // Симуляторная модель шин (§12.32) считает трение САМА, в шаге
            // 3б. Контроллеру тогда остаются луч, подвеска и вращение
            // колёс, а его собственное трение выключается целиком: без
            // этого силы легли бы дважды.
            //
            // Выключается оно нулевой боковой жёсткостью. Это не хитрость:
            // в `update_friction` вся ветка клампа стоит под
            // `if wheel.side_impulse != 0.0`, поэтому при нулевой боковой
            // контроллер не трогает и продольную, а нулевые `engine_force`
            // и `brake` не дают ей взяться.
            if t.tire_peak_slip > 0.0 {
                car.tire_engine = engine * 0.25;
                car.tire_brake = [
                    brake * 0.25,
                    brake * 0.25,
                    brake * 0.25 + hb_brake * 0.5,
                    brake * 0.25 + hb_brake * 0.5,
                ];
                let wheels = car.controller.wheels_mut();
                for w in 0..4 {
                    wheels[w].engine_force = 0.0;
                    wheels[w].brake = 0.0;
                    wheels[w].side_friction_stiffness = 0.0;
                }
            }
        }
        ...
        for i in 0..n {
            ...
            // 3б. СИМУЛЯТОРНАЯ МОДЕЛЬ ШИН (§12.32): кривая увода и круг
            // сцепления. При `tire_peak_slip == 0` ветка не исполняется, и
            // машину целиком считает встроенный контроллер — тот же код,
            // те же биты. Аркада ставит ноль.
            //
            // Почему сила считается ЗДЕСЬ, а не подменой чисел на входе
            // контроллера. У контроллера боковая сила упирается в
            // `friction_slip * Fz` и держится там до любого угла — потолок,
            // а не кривая. Опустить `friction_slip` под кривую нельзя: тот
            // же потолок делит бюджет с продольной, и продольная гасится
            // тем же множителем (`skid_info`), то есть тормоз пропал бы
            // вместе с боковой. Поэтому трение контроллера выключено
            // целиком (шаг 1), а обе силы кладутся здесь, в тех же пятнах
            // контакта и по тем же осям, которые построил его луч.
            if t.tire_peak_slip > 0.0 {
                let mass = body.mass();
                let com = body.center_of_mass();
                let up_ws = pose.rotation * Vector::new(0.0, 1.0, 0.0);
                // Опорная нагрузка колеса — статическая четверть веса.
                // Игрового числа тут нет: только масса машины и g.
                let fz_ref = mass * 9.81 * 0.25;
                let ng = if on_ground > 0 { on_ground as f32 } else { 1.0 };
                let mut out_fwd = [0.0f32; 4];
                let mut out_side = [0.0f32; 4];
                for w in 0..4 {
                    let (info, fz, axle_ws) = {
                        let wl = &car.controller.wheels()[w];
                        (
                            *wl.raycast_info(),
                            wl.wheel_suspension_force.min(t.max_suspension_force),
                            wl.axle(),
                        )
                    };
                    if !info.is_in_contact || fz <= 0.0 {
                        continue;
                    }
                    // Оси пятна контакта строятся ровно как в контроллере:
                    // ось колеса без нормальной составляющей, продольная —
                    // их векторное произведение.
                    let nrm = info.contact_normal_ws;
                    let lat = (axle_ws - nrm * axle_ws.dot(nrm)).normalize_or_zero();
                    let lng = nrm.cross(lat).normalize_or_zero();
                    let v = body.velocity_at_point(info.contact_point_ws);
                    let v_long = v.dot(lng);
                    let v_side = v.dot(lat);
                    let slip = (v_side / v_long.abs().max(TIRE_V_REF)).atan().abs();

                    // Пик сцепления ЭТОГО колеса. Чувствительность к
                    // нагрузке — то, без чего перенос веса не значит
                    // ничего: сумма Fz от него не меняется, и без падения
                    // mu с ростом Fz гружёное колесо возвращало бы ровно
                    // столько, сколько потеряло разгруженное.
                    let load = 1.0 - t.tire_load_sens * (fz / fz_ref - 1.0);
                    let mut mu = t.friction_slip * grip * load.clamp(0.25, 1.6);
                    if w >= 2 {
                        mu *= t.tire_rear_grip;
                    }
                    if mu < 0.0 {
                    ...
                    // Продольная: тяга либо тормоз. Ветки и их порядок —
                    // те же, что в контроллере: тяга старше тормоза, а
                    // тормоз не отдаёт больше, чем нужно, чтобы остановить
                    // пятно (иначе он погнал бы машину назад).
                    let f_long_cmd = if car.tire_engine != 0.0 {
                        car.tire_engine
                    } else if v_long != 0.0 {
                        let need = v_long.abs() * mass / (ng * DT);
                        -v_long.signum() * need.min(car.tire_brake[w])
                    } else {
                        0.0
                    };
                    let f_long_cap = f_long_cmd.clamp(-cap, cap);

                    // Кривая увода и круг сцепления. Круг — общий бюджет:
                    // сумма векторов не может выйти за mu*Fz, и лишнее
                    // режется ОБЕИМ, пропорционально. Отсюда и «тормозишь
                    // в пол — на поворот не остаётся», и обратное.
                    let f_lat_curve = cap * tire_curve(slip, t.tire_peak_slip, t.tire_tail);
                    let demand =
                        (f_long_cap * f_long_cap + f_lat_curve * f_lat_curve).sqrt();
                    let k = if demand > cap { cap / demand } else { 1.0 };
                    let f_long = f_long_cap * k;
                    let mut f_lat = f_lat_curve * k;

                    // Больше, чем нужно, чтобы убрать боковой снос, шина не
                    // даёт: иначе на околонулевом сносе она перекидывает
                    // машину через ноль и дрожит. Число то же, что считает
                    // контроллер (`side_friction_stiffness` на 0,2 от
                    // эффективной массы), и ручник роняет его ТОЙ ЖЕ
                    // формулой — то есть `handbrake_side_drop` из каталога
                    // сохраняет ровно прежний смысл и прежний замер.
                    //
                    // Ронять ручником саму КРИВУЮ пробовали и отказались:
                    // измерено, что при падающей кривой это разворачивает
                    // все пять машин на 86-88 град за полторы секунды и
                    // против прямого множителя, и против его корня. У
                    // падающей кривой обратная связь положительная: сорвав
                    // заднюю ось, ручник уводит её в спад, спад уводит
                    // дальше. Прежний потолок трения это скрывал. То есть
                    // симулятор оставляет ручник тем, чем он в нём и был по
                    // 12.23: машину возят рулём и газом, а не ручником.
                    let hb_w = if w >= 2 { hb_in } else { 0.0 };
                    let hold = t.side_friction_stiffness * grip
                        * (1.0 - (1.0 - t.handbrake_side_drop) * hb_w)
                        * 0.2 * mass * v_side.abs() / DT;
                    if f_lat > hold {
                        f_lat = hold;
                    }

                    out_fwd[w] = f_long * DT;
                    out_side[w] = -v_side.signum() * f_lat * DT;

                    // Продольная кладётся в пятно контакта — оттуда и
                    // берётся перенос веса при разгоне и торможении, он у
                    // лучевой подвески настоящий.
                    if out_fwd[w] != 0.0 {
                        body.apply_impulse_at_point(
                            lng * out_fwd[w],
                            info.contact_point_ws,
                            false,
                        );
                    }
                    // Боковая — в точку, поднятую к высоте центра масс на
                    // (1 - tire_roll_couple). Тем же приёмом контроллер
                    // держит `roll_influence = 0,1`, из-за чего у него
                    // поперечного переноса веса почти нет вовсе.
                    if out_side[w] != 0.0 {
                        let h = up_ws.dot(info.contact_point_ws - com);
                        let pt = info.contact_point_ws
                            - up_ws * (h * (1.0 - t.tire_roll_couple));
                        body.apply_impulse_at_point(lat * out_side[w], pt, false);
                    }
                }
                // Наружу уезжают НАШИ импульсы: иначе `forward_impulse` и
                // `side_impulse` в CarOut врали бы нулём.
                let wheels = car.controller.wheels_mut();
                for w in 0..4 {
                    wheels[w].forward_impulse = out_fwd[w];
                    wheels[w].side_impulse = out_side[w];
                }
            }
        }
