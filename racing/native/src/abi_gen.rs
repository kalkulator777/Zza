...
const _: () = assert!(size_of::<CarEffect>() == CarEffect::SIZE);
const _: () = assert!(offset_of!(CarEffect, stun) == 0);
const _: () = assert!(offset_of!(CarEffect, spin_rate) == 4);
const _: () = assert!(offset_of!(CarEffect, drift_reset) == 8);
const _: () = assert!(offset_of!(CarEffect, boost_add) == 12);
const _: () = assert!(offset_of!(CarEffect, speed_drop) == 16);
const _: () = assert!(offset_of!(CarEffect, grip_drop) == 20);
const _: () = assert!(offset_of!(CarEffect, push_x) == 24);
const _: () = assert!(offset_of!(CarEffect, push_y) == 28);
const _: () = assert!(offset_of!(CarEffect, push_z) == 32);
const _: () = assert!(offset_of!(CarEffect, shift_x) == 36);
const _: () = assert!(offset_of!(CarEffect, shift_y) == 40);
const _: () = assert!(offset_of!(CarEffect, shift_z) == 44);
...
/// Массив внешних воздействий. Хозяин пишет сюда напрямую ПЕРЕД шагом; модуль читает их в шаге и ОБНУЛЯЕТ запись после него. Адрес отдаёт rp_effects_ptr().
pub static mut EFFECTS: [CarEffect; MAX_CARS] = [CarEffect::INIT; MAX_CARS];
