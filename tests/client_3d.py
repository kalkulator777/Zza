...
# на экране занимает TILE_PX x TILE_PX*sin(57°) пикселей. При базовых 64 px
# (4.1) это 64 x 53.7 = 3435 пикселей, то есть 3435/(1920*1080) = 0.166 п.п.
# кадра; при прежних 48 было 1932 пикселя и 0.093 п.п.
#
# Порог падения черноты 5 п.п. — это 30 клеток; проверка не уходит с
# маршрута, пока в кадре исходной точки не откроется ENOUGH_NEW клеток, то
# есть запас 1.68 (не тройной, как в 2D: там часть новых клеток гасила
# виньетка, а здесь её нет вовсе).
#
# ЧИСЛО КЛЕТОК СЧИТАЕТСЯ ОТ МАСШТАБА, А НЕ СТОИТ ЛИТЕРАЛОМ: масштаб вырос с
# 48 до 64 px, кадр стал 30x16.9 клетки вместо 40x22.5 (600 клеток против
# 900), и прежние 90 клеток просто перестали набираться — замерено,
# открывалось 75 при исправном тумане и падении черноты 15 п.п. втрое выше
# порога. Красный от масштаба, а не от регрессии.
TILE_PX = 64                   # 4.1, базовый масштаб рендера
DROP_MIN = 0.05
CELL_PP = (TILE_PX * TILE_PX * math.sin(math.radians(57.0))
           / (1920.0 * 1080.0) * 100.0)      # п.п. кадра на одну клетку
ENOUGH_NEW = int(round(DROP_MIN * 100.0 / CELL_PP * 1.68))
