...
def t_two_answers():
    ...
    for atk in ("обвал", "залп"):
        print("    %-9s" % atk + "".join("%12d" % table[(atk, m)]
                                         for m in moves))
    note("как читать таблицу", "урон за одну атаку; бег 5.0 кл/с даёт "
         "%.2f клетки за %d тиков замаха" % (W.SPEED_RUN * boss.SLAM_WIND / 30.0,
                                             boss.SLAM_WIND))
    ...
    check(table[("залп", "вбок")] == 0 and table[("залп", "отступить")] > 0,
          ...
          "вбок %d, отступить %d — снаряд 12 кл/с догоняет бегущего по прямой"
...
def t_killable():
    ...
    check(t_up > 0 and hp_up > hp_bare,
          "набор апгрейдов платит ВЫЖИВАНИЕМ: с ним игрок выходит целее",
          "голый %d hp из 100, с набором %d hp — Ветер (откат рывка %d -> %d "
          "тиков) даёт уход на каждый замах, а не через раз"
          % (hp_bare, hp_up, combat.DASH_CD_TICKS,
             items.dash_cd(type("E", (), {"ups": [0, 0, 0, 1]})(),
                           combat.DASH_CD_TICKS)))
