...
# лоб в лоб, сближаются на 2 * 3.2/30 = 0.213 клетки за тик, а расходятся
...
def t_stack():
    ...
    check(on_w <= STACK_MAX,
          "с расталкиванием стопка не больше %d тел" % STACK_MAX,
          "худшая %d (было %d, в %.1f раза меньше)"
          % (on_w, off_w, off_w / float(on_w or 1)))
