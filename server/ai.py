...
# различать виды ЗНАЧЕНИЕМ kind, потому что ведут они себя противоположно, а
# рисуются пока одинаково. Соответствие ставится в make_enemy — одним
# местом, чтобы поведение и вид на проводе не разъехались.
AI_KIND = {AI_RANGED: K_ENEMY_RANGED}

# --- общие числа ----------------------------------------------------------
# Скорость врага НИЖЕ бега игрока (7.5 кл/с, 4.2) намеренно: убежать можно
# всегда, опасна толпа и окружение, а не догонялки.
#
# ПОДНЯТЫ ТЕМ ЖЕ МНОЖИТЕЛЕМ, ЧТО И БЕГ (world.SPEED_K = 1.5), а не заново
# подобраны. Отношения к игроку сохраняются точно, и именно они — контракт:
# рубака 0.64 бега игрока, стрелок 0.56. Подбирай враги по отдельности —
# и обещание «убежать можно всегда» держалось бы на честном слове, а так
# оно держится на арифметике: 4.8 < 7.5 ровно настолько же, насколько
# 3.2 < 5.0. Проверяется числом в tests/ai_check.py.
MELEE_SPEED = 4.8       # было 3.2
RANGED_SPEED = 4.2      # было 2.8
...
# идёт со скоростью 4.8 кл/с, то есть 18 / 4.8 = 3.75 с = 113 тиков. Меньше —
# и враг разворачивается на полпути, то есть шум ничего не притягивает.
# Оставлено 180 тиков (6 с): при прежних 3.2 кл/с запас был 6%, теперь 59%.
# Уменьшать его до 120 — значит менять баланс погони заодно с темпом, а это
# отдельная работа; в большую сторону порог безопасен по построению.
#
# ЧТО ИЗМЕНИЛ ТЕМП: шум стал ОПАСНЕЕ ровно в полтора раза. Громкость 8.2
# меряется в клетках пути и не изменилась, а добегают эти клетки теперь за
# 3.75 с вместо 5.6 — на 1.85 с раньше.
ALERT_TICKS = 180
...
# --- ГДЕ ТЕЛА РАСТАЛКИВАЮТСЯ (и почему не здесь) ------------------------
# Раньше сущности были друг для друга проницаемы, волна вела всех в одни и
# те же центры клеток, и у игрока стояло ДО 16 ТЕЛ В КРУГЕ РАДИУСА 0.35 —
# толпа рисовалась одним кружком с шестнадцатью полосками здоровья.
#
# Лечится это в physics.py, а не в ИИ, и вместе с подталкиванием на углах
# (4.2a), а не вместо: оба механизма — поперечный ход, и первая попытка,
# сложившая расталкивание в vx/vy, уронила долю дошедших со 100% до
# 94.5-96%. Сложенное в скорость расталкивание голосует в решении
# подталкивания (allow_dir и остаток шага) и в устье коридора запирает его
# в сторону ОТ полосы прохода 1 - 2r = 0.30. Поэтому ИИ по-прежнему
# выставляет ТОЛЬКО рулёжку, а расталкивание идёт отдельным смещением:
# physics.separate + аргумент push у move_circle, приоритет у прохода.
# Замер — tests/physics_crowd.py и tests/physics_gap.py (раздел 6).
...
# Выключатель ДЛЯ СТЕНДОВ, а не для игры. Нужен потому, что стенды
# клиента (tests/client_*.py) поднимают НАСТОЯЩИЙ play.py отдельным
# процессом и до его сущностей не дотягиваются: одинокий ходок, который
# меряет туман в кадре, теперь просто погибает, и красным становится
# исправная проверка исправного тумана. Переменная читается один раз при
# импорте, в тике её нет.
...
def make_enemy(w, x, y, ai_kind):
    ...
    e = w.spawn(AI_KIND.get(ai_kind, K_ENEMY), x, y,
                r=ENEMY_R, speed=speed, hp=hp, hp_max=hp)
    e.ai = ai_kind
    return e
