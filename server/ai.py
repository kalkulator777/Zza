...
# --- ПОЧЕМУ ТЕЛА НЕ РАСТАЛКИВАЮТСЯ (замер, а не забывчивость) -----------
# Физика толкает тело только СТЕНАМИ: сущности друг для друга проницаемы, а
# волна ведёт всех в одни и те же центры клеток. Замерено на этаже 5, 8
# этажей, врагов у игрока 16: до 16 ТЕЛ В КРУГЕ РАДИУСА 0.35, то есть вся
# ближняя толпа рисуется одним кружком с шестнадцатью полосками здоровья.
#
# Расталкивание написано и замерено: слипание уходит начисто (худшая стопка
# 16 -> 1 тело), но ДОЛЯ ДОШЕДШИХ падает со 100% до 94.5-96%. Причина не в
# силе толчка и не в его направлении — проверены полный толчок, только
# поперечный и запрет расталкивания у стен, все три дают одно и то же.
# Причина в 4.2a: physics.move_circle берёт поперечное направление для
# подталкивания либо любое (когда своего поперечного хода нет), либо строго
# совпадающее со своим — allow_dir = _sign(sdy), «не спорить с рулёжкой».
# Расталкивание и ЕСТЬ поперечный ход, и в устье коридора оно запирает
# подталкивание в сторону ОТ полосы прохода (1 - 2r = 0.30 клетки). Пары
# врагов вставали в полклетки друг от друга насмерть — ровно то залипание,
# которое 4.2a лечило.
#
# Разрешать это надо в physics.py (чужой файл) и с отдельным замером 4.2a,
# а не тихо здесь. Пока — читаемость толпы в отчёте как вопрос к контракту.
