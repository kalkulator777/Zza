...
class Entity(object):
    ...
    __slots__ = ("id", "kind", "x", "y", "vx", "vy", "hp", "hp_max",
                 ...
                 # ОПЫТ И УРОВНИ (items.py, 8.4a). Серверные по тому же
                 # доводу, что и ups: 4.3 даёт ровно десять полей, и ни
                 # «сколько у меня опыта», ни «какой у меня уровень» в них
                 # не входят. На провод уровень уходит событием lvl, а его
                 # СЛЕДСТВИЕ — новым апгрейдом в наборе, то есть сообщением
                 # build (11.7), которое комната и так шлёт при изменении.
                 # Правки proto.py и клиента это не требует ни одной.
                 "xp", "lvl", "lvl_owed", "offer")
    ...
    def __init__(self, eid, kind, x, y):
        ...
        self.xp = 0             # опыт СВЕРХ последнего уровня (8.4a)
        self.lvl = 0            # уровень; 0 — ни одного не взято
        self.lvl_owed = 0       # уровней, выбор по которым ещё не выложен
        self.offer = 0          # номер лежащего сейчас выбора (0 — нет)
...
class World(object):
    def __init__(self, grid, seed=1, floor=1, spawns=None, stairs=None):
        ...
        # Очередь «кому выложить выбор за уровень» (8.4a). Очередь, а не
        # перебор игроков каждый тик, ровно по тому же доводу, что и
        # pickups: выкладка МЕНЯЕТ состав мира, значит она обязана идти вне
        # прохода по словарю, а пустая очередь стоит одно сравнение.
        self.level_q = []
        # ОБЩИЙ СЧЁТЧИК ВОСКРЕШЕНИЙ ГРУППЫ (8.5, дыра 11.8). Живёт в мире, а
        # не в игроке, потому что это ресурс ГРУППЫ: смерть напарника —
        # убыток всем. Вывод числа — в items.REVIVE_BASE.
        self.revives = items.REVIVE_BASE
    ...
    def enter_floor(self, floor, grid, spawns, stairs):
        ...
        del self.pickups[:]
        # очередь уровней тоже: выложенные предметы сейчас исчезнут вместе с
        # врагами, а долги пересобираются ниже, по e.lvl_owed
        del self.level_q[:]
        ...
        for e in list(self.entities.values()):
            ...
            e.btn = 0
            if e.flags & F_DEAD:
                # 8.5 + дыра 11.8: воскрешение больше НЕ бесплатно. Оно
                # тратит общий заряд группы и возвращает половину здоровья
                # (вывод — items.REVIVE_BASE / REVIVE_SHARE). Зарядов нет —
                # дух остаётся духом: летает, видит, слышит, но не дерётся и
                # не светит туман (4.4). Перед серым экраном не сидит никто,
                # ради чего 8.5 и писался, — но смерть перестала быть
                # полным лечением с переносом к лестнице.
                if items.spend_revive(self):
                    e.flags &= ~F_DEAD
                    hp = int(e.hp_max * items.REVIVE_SHARE)
                    e.hp = hp if hp > 1 else 1
                    self.event("rev", a=e.id, b=self.revives,
                               n=self.revives)
            # Долг по уровню переживает этаж: предметы старого этажа убраны
            # выше вместе с врагами, значит выложенный выбор пропал, а
            # заработанный уровень пропасть не имеет права.
            e.offer = 0
            if e.lvl_owed > 0:
                items._queue_level(self, e)
