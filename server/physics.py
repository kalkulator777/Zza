...
# времени: r / скорость = 0.35 / 7.5 = 0.047 с = 1.4 тика при беге 7.5 кл/с
# (4.2; на прежних 5.0 было 2.1 тика). Скорость сущности подталкивание не
# повышает никогда, поэтому от роста темпа тут не ломается ничего: быстрее
# тело — быстрее и вытаскивание его в створ.
...
# 0.35 / (0.5 * 4.8 / 30) = 4.4 тика = 0.15 с при скорости рубаки 4.8 кл/с
# (4.2; на прежних 3.2 это было 6.6 тика) — «за несколько тиков», а не за
# один. Мгновенное разведение и было тем, что порвало проход: тело за один
# тик улетало из полосы створа. Доля от ШАГА, а не клетки в секунду, —
# поэтому от роста темпа отношение «толчок против шага» не сдвинулось ни на
# сколько, сдвинулось только время разведения, и в лучшую сторону.
PUSH_SHARE = 0.5
